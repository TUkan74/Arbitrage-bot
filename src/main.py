...
def get_vol_exchange(id, days, base_curr):
    ...
    exchange_vol_response = gecko_client.get_response(f"/exchanges/{id}/volume_chart", vol_params)
...
def run_bot(
    ...
    df_ex_all = fetch_tickers_for_multiple_exchanges(base_id,base_curr, target_curr,country)
    
    # Collect data every minute    
    while True:
        time.sleep(60)
        df_new = fetch_tickers_for_multiple_exchanges(base_id,base_curr, target_curr,country)
