...
@pytest.mark.parametrize("endpoint,expected_base", [
...
def test_base_url_selection(binance, endpoint, expected_base):
    """Test that the correct base URL is selected based on the endpoint"""
    with patch('requests.request') as mock_request:
        mock_request.return_value.json.return_value = {}
        mock_request.return_value.status_code = 200
        binance._make_request(HttpMethod.GET, endpoint)
        # Check that the request was made with the correct URL
        called_args = mock_request.call_args[0]
        called_url = called_args[1]  # URL is the second positional argument
        assert called_url.startswith(expected_base), f"Expected URL to start with {expected_base}, but got {called_url}"

def test_get_ticker(binance):
    ...
    with patch.object(binance, '_make_request') as mock_request:
        mock_request.return_value = mock_response
        result = binance.get_ticker("BTC/USDT")
...
def test_get_order_book(binance):
    ...
    with patch.object(binance, '_make_request') as mock_request:
        mock_request.return_value = mock_response
        result = binance.get_order_book("BTC/USDT", limit=2)
...
def test_get_trading_fees(binance):
    ...
    with patch.object(binance, '_make_request') as mock_request:
        mock_request.return_value = mock_response
        result = binance.get_trading_fees()
